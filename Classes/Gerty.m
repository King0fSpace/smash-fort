...
{
	//[self initWithShape:shape file:@"Tamagachi.png"];
    ...
    if (tamagachi1Color == TAMAGACHI_1_RED) {
       ...
       [self initWithShape:shape spriteFrameName: @"Tamagachi Z-500 yellow.png"];
        ...
        //If tamagachi1Color is undeterminable choose Red anyway
        [self initWithShape:shape spriteFrameName: @"Tamagachi Z-200 red.png"];
    }
    ...
    shape->layers = 2;  shape->u = 5.0;  shape->e = 0.0;  
	...
	//Handle collisions
	shape->collision_type = kGertyCollisionType;
...
{
    gertySlotAnimationIsRunning = YES;
    ...
    else if (doesTheNextMarbleHaveAHighChanceOfBeingASuperMarble1 == NO) {
...
{    
    NSLog (@"doesTheNextMarbleHaveAHighChanceOfBeingASuperMarble1 = %i", doesTheNextMarbleHaveAHighChanceOfBeingASuperMarble1);
...
{
    CDXAudioNode *audioSlotMachineSpinning = [[self children] objectAtIndex:14];
    [audioSlotMachineSpinning play]; 
...
{        
    ...
    //[(Game*)_game player2IsTheWinnerScript];
    
    audioGerty1BeingDestroyed = [[self children] objectAtIndex:12];
    [audioGerty1BeingDestroyed play]; 
