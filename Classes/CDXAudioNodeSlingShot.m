...
@implementation CDXAudioNodeSlingShot

@synthesize earNode;
@synthesize playModeSlingShot;
...
- (id)initWithSoundEngine:(CDSoundEngine *)se sourceId:(int)sId {
    ...
    if ((self = [super init])) {
        ...
        playMode = kAudioNodeSinglePlaySlingShot;
...
- (void)play {
    ...
    switch (playMode) {
        case kAudioNodeSinglePlaySlingShot:
            sound.looping = NO;
            break;
        case kAudioNodeLoopSlingShot:
            sound.looping = YES;
            break;
        case kAudioNodePeriodicLoopSlingShot:
            sound.looping = NO;
